...
# 화면 위, 아래쪽 벽에 맞으면 튕겨나가게 설정
    if gdy <= max(brList[i].y+40+3 for i in range(len(brList))) or gdy >= (800-50-3):       # 동전 y좌표가 화면의 위 또는 아래 경계선에 부딪히면
            iy = -iy        # y좌표의 증감을 바꿈
# 화면 왼, 오른쪽 벽에 맞으면 
...
while isRunning:        # 참 동안 반복
    fps = clock.tick(10)        # 1초당 frame수 30으로 지정
    ...
    for brxy in brList:
        if gdy <= max(brxy.y+40+3 for i in range(len(brList))):
            brxy.y = -20
            print('부딪힘')
