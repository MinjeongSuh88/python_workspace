...
class MyApp(QWidget):
    ...
    def keyPressEvent(self,e):
        ...
        if e.key() == Qt.Key_Space: # QT.Key_Space :키보드 스페이스바의 키값
            t = threading.Thread(target = self.moveKitty)
            t.start()

    def moveKitty(self): # 화면 움직이는 애와 
        print('스페이스 눌림')
        for i in range(10):
            time.sleep(0.01)
            self.label.move(self.label.x()+1, -1*(self.label.x()+1)**2+800*(self.label.x()+1)-1200)
        # for i in range(10):
        #     time.sleep(0.01)
        #     self.label.move(self.label.x()+5, self.label.y()+10)
