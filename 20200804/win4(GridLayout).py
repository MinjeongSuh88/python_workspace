...
# QWidget
# 위잿은 화면에 표시할 수 있는 것을 목적
# 윈도우나 버튼, 모든 위젯, 화면에 무언가를 표시하거나
# 키보드 or 마우스에서 사용자의 입력을 받아들이는 것(버튼, 슬라이드, 뷰, 대화 상자 등)
# 사용자의 상호 작용을 나타내는 사각형 영역

# QMainWindow
# 메인 창에서는 최상위 위젯임(메뉴바, 도구모음, 상태표시줄을 포함하는 미리 정의된 레이아웃 가지고 있음)
# 창은 부모/자식의 상단이며 일반적으로 제목 표시줄과 테두리를 표시

# QDialog
# 특수한 종류의 창(다른 이름으로 저장 등)으로 보통 일시적
# 알림, 입력, 선택을 위해

class MyWidget(QWidget):
    def __init__(self,parent): # 여기서 parent는 MyRegister를 의미
        super().__init__()
    ...
    def initUI(self):
        ...
        grid.addWidget(self.btn,3,0) # 객체,행,열
